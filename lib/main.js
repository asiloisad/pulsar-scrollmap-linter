    ...
    return {
      ...
      getItems: ({ editor, cache }) => {
        ...
        for (const message of data) {
          const startRow = editor.screenPositionForBufferPosition(message.location.position.start).row;
          const endRow = editor.screenPositionForBufferPosition(message.location.position.end).row;
          const cls = message.severity;
          if (lastItem && lastItem.cls === cls && startRow <= (lastItem.end ?? lastItem.row) + 1) {
            lastItem.end = endRow;
          } else {
